...
The repository now uses a deterministic oracle (`test_oracle.py`) and no longer uses filename-based PASS/FAIL assignment. Each test is executed natively, and classification is derived from measured VCD signatures.

| Test / Signature Scope | Evaluated | Status | Notes |
|---|---|---|---|
| `rv32ui-p-add` (Smoke Test) | Yes | **PASS** | Validated explicit control signature: `x3 == 12` mapped directly from VCD register writes. |
| Other `rv32ui-p-*` tests | No | **INCONCLUSIVE** | ISA hex payloads excluded. Scaled functional verification is fully structurally supported by the oracle script when the repository is populated with genuine compiled RISC-V test binaries. |

**Current classification model:**
1. `PASS`: deterministic signature check passed (e.g., the add smoke program).
2. `FAIL`: simulation returned error or core failed basic execution signature checks.
3. `INCONCLUSIVE`: program executed, but harness lacks full architectural pass/fail contract for that test (avoids falsely assigning PASS to unsupported instructions or synthetic duplicates).

**Architectural Limits (Memory Interface Extrusion):**
The `load_store_unit.v` was successfully generated by the RAG LLM (see Trace), but the top-level stitcher (`rv32i_core.v`) was explicitly constrained not to include an external data memory interface (`data_mem_addr`, `data_mem_wdata`) to isolate and test the EDA pipeline's ability to self-heal internal CPU control paths. Load/Store instructions inherently evaluate to FAIL or INCONCLUSIVE structurally within the VCD harness context without port mappings.

**2. Benchmark Results**
Dhrystone benchmark claims are currently deferred until a non-dummy benchmark payload and a reproducible measurement harness are committed. The present `benchmarks/dhrystone.hex` placeholder is intentionally excluded from scoring.
